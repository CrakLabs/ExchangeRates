...
class IMFRepository implements RatesRepository
{
    ...
    private $url = "http://www.imf.org/external/np/fin/data/rms_five.aspx?tsvflag=Y";

    /**
     * @param CurlRepository $curlRepository
     */
    ...
    public function getRates(\DateTime $dates = null) {
        $csv = $this->curlRepository->get($this->url);;

        if (!$csv) {
            throw new \Exception('Cannot get the online currencies exchange rates');
        }
